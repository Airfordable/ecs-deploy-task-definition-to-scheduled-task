...
describe('filterUnrelatedTaskDefTargets', () => {
  ...
  it('Filters targets that are not related', async () => {
    ...
    const filtered = ecsCwe.filterUnrelatedTaskDefTargets(
      targets,
      newTaskDefArn
    );
  ...
  it('Errors on invalid taskDefArn', async () => {
    ...
    expect(() => ecsCwe.filterUnrelatedTaskDefTargets([], notArn)).toThrow(
      notArn
    );
    expect(() =>
      ecsCwe.filterUnrelatedTaskDefTargets([], invalidTaskArn)
    ).toThrow(invalidTaskArn);
  });
});
